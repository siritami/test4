...
get_apk() {
	...
	if [ -z "$version" ] && [ "$lock_version" != "1" ]; then
		if [[ $(ls revanced-cli-*.jar > /dev/null 2>&1) =~ revanced-cli-([0-9]+) ]]; then
			num=${BASH_REMATCH[1]}
			if [ $num -ge 5 ]; then
...
lspatch() {
	...
	ls /home/runner/work/test4/test4/src/ks.keystore
	if [ -z "$patch_file" ]; then
        red_log "[-] Missing patch APK"
        exit 1
    fi
	if [ -f "./download/$1.apk" ]; then
		java -jar lspatch.jar --embed "$patch_file" --sigbypasslv 2 --injectdex --keystore ["/home/runner/work/test4/test4/src/ks.keystore", null, "ReVanced Key", null] --output "./release/" "./download/$1.apk"
	else 
		red_log "[-] Not found $1.apk"
		exit 1
	fi
